...
stage('docker test') { 
        ...
        stage('Build Docker image'){
           steps {
           	    ...
           	    bat 'docker build -t  subscriberimage .'
		         }
             }
	     stage('Docker Login'){
            steps {
        ...
        stage('Docker Tag'){
            steps {
          	      ...
          	      bat 'docker tag subscriberimage  aruna708/subscriberimage:latest'
            }                
        }
        stage('Docker Push'){
	...
	//	 withCredentials([usernamePassword(credentialsId: 'dockerhub-credentials1', usernameVariable: 'aruna708', passwordVariable: 'Aruna708*')]) {
        ...
        //                echo "$Aruna708*" | docker login -u "$aruna708" --password-stdin 
         ...
         //          '''
          //      }
		   bat 'docker push aruna708/subscriberimage'
        ...
        stage('Docker deploy'){
            ...
            steps {
                bat 'docker run -itd -p  8081:8081 subscriberimage'
             }
        }
